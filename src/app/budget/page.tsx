...
import DeviceCarousel from '@/components/deviceItems/deviceCarousel';
...
export default function BudgetPage() {
  ...
  const fetchRecommendation = async () => {
    try {
      const { data } = await axios.post<AdPlan[]>('/api/gemini/deviceSort', {
        biz_type,
  ...
  const pricePerDevice = (device: string) =>
    device === 'IPTV' ? 15 : device === '버스정류장' ? 10 : 5;

  const toNum = (v: unknown) =>
    typeof v === 'number' ? v : parseInt(String(v ?? '0'), 10) || 0;

  const priceCalculateBy = (list: AdPlan[], idx: number) => {
    const item = list?.[idx];
    if (!item) return 0;
    return (
      pricePerDevice(item.device) *
      toNum(list?.[0]?.deviceCount) *
      (item.timeSlots?.length ?? 0) *
      3 *
      toNum(item.impressions)
    );
  };
  ...
  return (
    <div className="container mt-3">
      <section className="shadow-section mb-[72px] flex w-[1192px] flex-col items-center justify-center gap-5 rounded-[20px] bg-white px-[60px] pt-3 pb-[60px]">
        ...
        />
        {isLoading ? (
          <Loading headLine={'예산 분배 최적화를 진행중이에요'} />
        ) : (
          <>
            {step === 0 && (
              <>
                ...
                <div className="flex w-full flex-row justify-between">
                    ...
                    <div className="flex flex-row gap-2">
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          {cityFocus && (
                            <div className="ring-line-assistive bg-normal-inverse absolute top-[50px] left-0 z-20 flex h-fit max-h-[358px] w-full flex-col overflow-y-scroll rounded-xl p-1.5 ring-1">
                  ...
                  <section>
                    ...
                    <div className="text-BodySM text-text-assistive">
                      광고 일정은 최소 1주 이상부터 선택 가능하며, 금일 7일
                      이후부터 신청 가능해요.
                    </div>
                    <div className="mt-6 mb-9 flex flex-row gap-2">
                      ...
                      >
                        <button
                          ...
                          onClick={() =>
                            setStartCalendarFocus(!startCalendarFocus)
                          }
                        >
            ...
            {step === 1 && (
              <>
                ...
                <div className="flex h-[482px] w-full flex-row justify-between gap-9">
