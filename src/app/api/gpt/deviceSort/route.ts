...
export async function POST(req: Request) {
  try {
    ...
    const { biz_type, budget, days } = body;
    const prompt = {
      instruction: `${biz_type} 업종, 예산 ${budget}원, ${days}기간을 기준으로 광고 집행 계획을 만들어라.`,
      requirements: {
        rules: [
          '광고 매체는 엘리베이터 TV, 버스정류장, IPTV 중 2가지만 선택할 것',
          '예산은 무조건 넘으면 안되고, 최대한 사용하는 방식으로 최적화',
          ...
          '모든 값은 문자열 처리',
          '기기 선택 이유에 대해서는 한줄로 간단하고 명료하게 작성해줘',
          '기기 이름 필드 포함',
          'JSON만 출력할 것, 설명 금지',
        ],
      },
